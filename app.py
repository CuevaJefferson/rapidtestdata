...
building_block_name = st.sidebar.selectbox(options=[x.split(".")[0] for x in os.listdir(path)], label="Building Block")
# st.sidebar.text(f"uploaded scenarios: {}")
mock_data = st.sidebar.file_uploader("Mock Data", type="json", accept_multiple_files=True, key=f"mock_uploader_{st.session_state.uploader_key}")
input = st.sidebar.file_uploader("Input", type="json", accept_multiple_files=False, key=f"input_uploader_{st.session_state.uploader_key}")
output = st.sidebar.file_uploader("Output", type="json", accept_multiple_files=False, key=f"output_uploader_{st.session_state.uploader_key}")
...
if st.session_state.add_scenario:
    ...
    with open(f'Documentations/{building_block_name}.docx', "rb") as docx_file:
        documentation_mkdown = mammoth.convert_to_html(docx_file).value
    print(documentation_mkdown)
    if mock_data is not None:
        for mock_data_item in mock_data:
            add_mock_data_json += mock_data_item.getvalue().decode( "utf-8" )
    if input is not None:
    ...
    print(f"mock: {mock_data_json}")
    print(f"output: {output_json}")
    response=chain.stream({"documentation":documentation_mkdown, "inputjson":input_json, "outputjson": output_json, "mockdata":mock_data_json, "question": question})
